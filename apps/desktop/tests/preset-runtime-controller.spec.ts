...
class FixtureInstaller implements PresetRuntimeInstaller {
  ...
  async install(
    ...
    if (presetId === 'product-video-director') {
      ...
      await writeFile(join(root, 'package.json'), '{"name":"hyperframes","version":"0.7.109"}\n')
    ...
    await writeFile(join(echarts, 'package.json'), '{"name":"echarts","version":"6.1.0"}\n')
    ...
    await writeFile(join(playwright, 'package.json'), '{"name":"playwright","version":"1.61.1","main":"index.cjs"}\n')
    await writeFile(join(playwright, 'index.cjs'), `module.exports={chromium:{executablePath:()=>${JSON.stringify(browser)}}}\n`)
  }
}
...
describe('Preset runtime controller', () => {
  ...
  it('moves the video runtime from detected missing to verified ready after one confirmed install', async () => {
    ...
    expect(missing.dependencies.find(item => item.id === 'hyperframes')?.state).toBe('missing')
    ...
    expect(ready.dependencies.every(item => item.state === 'ready')).toBe(true)
  })
  ...
  it('installs the managed report libraries but keeps missing Python as an explicit manual requirement', async () => {
    ...
    expect(result.dependencies.find(item => item.id === 'python')).toMatchObject({
      state: 'missing', installable: false,
    })
    expect(result.dependencies.find(item => item.id === 'playwright')?.state).toBe('ready')
    expect(result.dependencies.find(item => item.id === 'chromium')?.state).toBe('ready')
